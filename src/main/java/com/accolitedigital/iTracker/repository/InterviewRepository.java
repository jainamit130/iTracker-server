...
//    List<Interview> findByEmailAndStartDateBetween(String email, long startDate);
    @Query("SELECT i FROM Interview i WHERE " +
            "(:skill is null or i.skill = :skill) and " +
            "(:round is null or i.round = :round) and " +
            "(i.startDate between :startDate and :endDate)")
    List<Interview> findBySkillAndRoundAndStartDateBetween
